...
### Grafana image renderer plugin and self-signed or custom CA certificates

If you are using the Grafana image renderer with a Grafana server that uses a self-signed certificate or a certificate signed by a custom CA (e.g. a company-internal CA) that is not known to the nss trust store shipped with headless chrome, rendering images will fail and you will see messages like this in the Grafana log:
